...
def depoist():
    while True:
        amount = input("Enter a amount to depoist: $")
        if amount.isdigit():
            amount = int(amount)
            if amount > 0:
                break
...
def get_bet():
    while True:
        value = input("Enter a amount to bet on line: ")
        if value.isdigit():
            value = int(value)
...
def spin(balance):
    ...
    print(f"Yor are betting ${bet} on {lines} lines. Your total bet amount is ${total_bet} ")
